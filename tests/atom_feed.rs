...
fn test_parse_example_feed() {
    ...
    assert_eq!(
        feed.logo.as_deref(),
        Some("https://blog.henritel.com/media/favicon.svg")
    );
    assert_eq!(
        feed.subtitle.as_deref(),
        Some("The latest news from HenriTEL")
    );
    ...
    assert_eq!(
        first.link.href,
        "https://blog.henritel.com/tech/embedded-systems-level-1"
    );
    ...
    let android = feed
        .entries
        .iter()
        .find(|e| e.title == "Android Sucks on Google Phones")
        .unwrap();
